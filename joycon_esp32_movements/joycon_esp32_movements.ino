...
void processGamepad(ControllerPtr ctl) {
  ...
  static bool prevJoystickUp = false;
  bool currentJoystickUp = (ctl->axisY() <= JOYCON_UP_THRESHOLD);
  if (currentJoystickUp && !prevJoystickUp) {
    Serial.println("Joystick: UP");
    // code for when left joystick is pushed up
  }
  prevJoystickUp = currentJoystickUp;

  //== LEFT JOYSTICK - DOWN ==//
  // Based on your actual reading: Down: axis L: -100, 440
  static bool prevJoystickDown = false;
  bool currentJoystickDown = (ctl->axisY() >= JOYCON_DOWN_THRESHOLD);
  if (currentJoystickDown && !prevJoystickDown) {
    Serial.println("Joystick: DOWN");
    // code for when left joystick is pushed down
  }
  prevJoystickDown = currentJoystickDown;

  //== LEFT JOYSTICK - LEFT ==//
  // Based on your actual reading: Left: axis L: -417, -30
  static bool prevJoystickLeft = false;
  bool currentJoystickLeft = (ctl->axisX() <= JOYCON_LEFT_THRESHOLD);
  if (currentJoystickLeft && !prevJoystickLeft) {
    Serial.println("Joystick: LEFT");
    // code for when left joystick is pushed left
  }
  prevJoystickLeft = currentJoystickLeft;

  //== LEFT JOYSTICK - RIGHT ==//
  // Based on your actual reading: Right: axis L: 305, -29
  static bool prevJoystickRight = false;
  bool currentJoystickRight = (ctl->axisX() >= JOYCON_RIGHT_THRESHOLD);
  if (currentJoystickRight && !prevJoystickRight) {
    Serial.println("Joystick: RIGHT");
    // code for when left joystick is pushed right
  }
  prevJoystickRight = currentJoystickRight;
  ...
  static bool prevUpLeft = false, prevUpRight = false, prevDownLeft = false, prevDownRight = false;
  
  bool currentUpLeft = (ctl->axisX() <= JOYCON_LEFT_THRESHOLD && ctl->axisY() <= JOYCON_UP_THRESHOLD);
  bool currentUpRight = (ctl->axisX() >= JOYCON_RIGHT_THRESHOLD && ctl->axisY() <= JOYCON_UP_THRESHOLD);
  bool currentDownLeft = (ctl->axisX() <= JOYCON_LEFT_THRESHOLD && ctl->axisY() >= JOYCON_DOWN_THRESHOLD);
  bool currentDownRight = (ctl->axisX() >= JOYCON_RIGHT_THRESHOLD && ctl->axisY() >= JOYCON_DOWN_THRESHOLD);
  
  if (currentUpLeft && !prevUpLeft) {
    Serial.println("Joystick: UP-LEFT");
  }
  if (currentUpRight && !prevUpRight) {
    Serial.println("Joystick: UP-RIGHT");
  }
  if (currentDownLeft && !prevDownLeft) {
    Serial.println("Joystick: DOWN-LEFT");
  }
  if (currentDownRight && !prevDownRight) {
    Serial.println("Joystick: DOWN-RIGHT");
  }
  
  prevUpLeft = currentUpLeft;
  prevUpRight = currentUpRight;
  prevDownLeft = currentDownLeft;
  prevDownRight = currentDownRight;
  ...
  // Uncomment the line below if you want to see all controller data constantly
  // dumpGamepad(ctl);
}
